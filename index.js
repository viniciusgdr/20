...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Vinicius\n' // Nama kamu
            + 'ORG:Vinicius;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558183064666:+55 81983064666\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '!'
blocked = []
limitawal = '999999999'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬❗❭ Aguarde!!!',
					success: '️❬ ✔ ❭ Sucesso',
					levelon: '❬ ✔ ❭ *leveling foi Ativado!',
					leveloff: ' ❬ X ❭  *leveling foi Desativado',
					levelnoton: '❬ X ❭ *leveling nunca foi ativo aq*',
					levelnol: '*erro',
					error: {
				stick: '*Bem, falhou. tente novamente*',
				Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS*',
					mod: '[❗] ESTE COMANDO È BLOQUEADO A TODOS*',
					benned: 'quem manda ser bloqueado, tomou no cu',
					ownerG: '[❗] Este é um Recurso Privado❌',
					ownerB: '[❗] Este é um Recurso Privado ❌',
					userB: `Olá!, Você não Está cadastrado no Meu Sistema!\n Cadastre-se asim: ${prefix}cadastrar seunome\n Exemplo: ${prefix}cadastrar vinicius`,
			...
			const mod = [ownerNumber,"558183064666@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558183064666@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558183064666@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558183064666@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('cara, nao fale essas coisas. mas tu é adm te perdoo')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} voce sera expulso`)
		setTimeout( () => {
		...
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('cara, nao fale essas coisas')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} voce sera expulso`)
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} voce sera expulso`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
...
//@darkYT					
    ...
    break
            ...
            case 'bodoh':
				...
				case 'cadastrar':
					client.updatePresence(from, Presence.composing)
					if (args.length < 1) return reply(`errou ai\nComando Correto : ${prefix}cadastrar vini\nEx : ${prefix}cadastrar Vinicius`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./src/user.json', JSON.stringify(user))
						client.sendMessage(from, `Registrado com Suscesso\nTotal de usuários ${user.length}\``, text, {quoted: mek})
					break
				case 'hunti':
					reply(mess.wait)
		...
		case 'bpfont':
			...
			break 
				...
				uptime = process.uptime()
                    client.sendMessage(from, `Velocidade do *ViniciusBOT*: *${latensi.toFixed(3)}* _Segundos_\n*O bot esteve ativo por*\n*${kyun(uptime)}*`, text, { quoted: mek})
                    break
				case 'delete':
				case 'apagar':
				case 'deletar':  
           ...
           case 'play2':
                reply(mess.wait)
                play = body.slice(9)
                anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?q=${play}&apikey=vinzapi`, {method: 'get'})
               if (anu.error) return reply(anu.error)
...
break
				...
				case 'deletar2':
					...
					if (mentioned.length > 1) {
						...
						mentions(`si fudeu kkk : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.deleteMessage(from, mentioned)
					}
					break
				case 'openanime':
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				case 'info':
					me = client.user
					uptime = process.uptime()
					teks = `*Número do bot* : @${me.jid.split('@')[0]}\n*Prefixo* : ${prefix}\n*Contatos bloqueados* : ${blocked.length}\n*O bot está ativo em* : ${kyun(uptime)}*`
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
					...
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Vinicius ou digite * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT VINICIUS ✨* 」\n*│+ wa.me/5581983064666*╰──────*「 *vini* 」*────*\n\n*_SE QUER SER ADMIN DO BOT VINICIUS_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/558183064666/*╰──────*「 *VINICIUSBOT* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO VINI BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
			...
			reply(mess.wait)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ TRANSMIÇÃO URGENTE ]\n\n${body.slice(4)}`})
						}
						reply('Transmissão enviada com sucesso')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ TRANSMISSÃO URGENTE ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão enviada com sucesso')
					}
					break
